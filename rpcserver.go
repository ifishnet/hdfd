...
var (
	...
	// data.
	gbtCoinbaseAux = &hdfjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &hdfjson.RPCError{
		Code:    hdfjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &hdfjson.RPCError{
		Code:    hdfjson.ErrRPCNoWallet,
...
// not needed.
func internalRPCError(errStr, context string) *hdfjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return hdfjson.NewRPCError(hdfjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *hdfjson.RPCError {
	return hdfjson.NewRPCError(hdfjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *hdfjson.RPCError {
	return hdfjson.NewRPCError(hdfjson.ErrRPCNoTxInfo,
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.HdfEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > hdfutil.MaxSatoshi {
			return nil, &hdfjson.RPCError{
				Code:    hdfjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := hdfutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &hdfjson.RPCError{
		...
		satoshi, err := hdfutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []hdfjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]hdfjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &hdfjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []hdfjson.Vout {
	voutList := make([]hdfjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout hdfjson.Vout
		vout.N = uint32(i)
		vout.Value = hdfutil.Amount(v.Value).ToHDF()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*hdfjson.TxRawResult, error) {
	...
	txReply := &hdfjson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(hdfutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(hdfutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := hdfjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := hdfutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := hdfjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &hdfjson.RPCError{
			Code: hdfjson.ErrRPCInternal.Code,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &hdfjson.RPCError{
				Code:    hdfjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*hdfjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result hdfjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = hdfjson.Bool(peer.Connected())
		...
		addrs := make([]hdfjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr hdfjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &hdfjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := hdfutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := hdfjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if *c.Verbosity == 1 {
		...
		rawTxns := make([]hdfjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &hdfjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		SoftForks: &hdfjson.SoftForks{
			Bip9SoftForks: make(map[string]*hdfjson.Bip9SoftForkDescription),
		},
	}
	...
	chainInfo.SoftForks.SoftForks = []*hdfjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &hdfjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := hdfjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a hdfjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*hdfjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]hdfjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := hdfutil.NewTx(tx)
		resultTx := hdfjson.GetBlockTemplateResultTx{
	...
	reply := hdfjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *hdfjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCClientInInitialDownload,
...
// chainErrToGBTErrString converts an error returned from hdfchain to a string
...
func handleGetBlockTemplateProposal(s *rpcServer, request *hdfjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &hdfjson.RPCError{
			Code: hdfjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*hdfjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hdfjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &hdfjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToHDF(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &hdfjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := hdfjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := hdfjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &hdfjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hdfjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*hdfjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &hdfjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &hdfjson.RPCError{
				Code: hdfjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &hdfjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         hdfutil.Amount(value).ToHDF(),
		ScriptPubKey: hdfjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]hdfjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]hdfjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]hdfjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &hdfjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     hdfutil.Amount(originTxOut.Value).ToHDF(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr hdfutil.Address, numToSkip, numRequested uint32) ([]*hdfutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hdfjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := hdfutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]hdfjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = hdfjson.ErrRPCTxAlreadyInChain

			default:
				code = hdfjson.ErrRPCTxRejected
			}
		}
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.ValidateAddressCmd)

	result := hdfjson.ValidateAddressChainResult{}
	addr, err := hdfutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hdfjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*hdfutil.AddressPubKeyHash); !ok {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCType,
	...
	pk, wasCompressed, err := hdfcec.RecoverCompact(hdfcec.S256(), sig,
		expectedMessageHash)
	if err != nil {
	...
	address, err := hdfutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a ifishnet extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]hdfjson.VersionResult{
		"hdfdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, hdfjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *hdfjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(hdfjson.Error); ok &&
			jerr.ErrorCode == hdfjson.ErrUnregisteredMethod {

			parsedCmd.err = hdfjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = hdfjson.NewRPCError(
			hdfjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *hdfjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *hdfjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*hdfjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return hdfjson.MarshalResponse(id, result, jsonErr)
}
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *hdfutil.Block, flags blockchain.BehaviorFlags) (bool, error)
