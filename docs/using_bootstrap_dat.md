...
5. [How do I use bootstrap.dat with hdfd?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Hdfd will download the
...
### 5. How do I use bootstrap.dat with hdfd?

hdfd comes with a separate utility named `addblock` which can be used to import
...
1. Stop hdfd if it is already running.  This is required since addblock needs to
   access the database used by hdfd and it will be locked if hdfd is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\Hdfd Suite\Hdfd\addblock" -i C:\Path\To\bootstrap.dat
```
