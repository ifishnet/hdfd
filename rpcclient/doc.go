...
Bitcoin RPC server that uses a hdfd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with hdfd (https://github.com/ifishnet/hdfd),
...
detected by type asserting the error in a *hdfjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*hdfjson.RPCError); ok {
  		switch jerr.Code {
  		case hdfjson.ErrRPCUnimplemented:
