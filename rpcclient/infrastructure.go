...
const (
	...
	// Hdfd represents a catch-all hdfd version.
	Hdfd
)
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *hdfjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *hdfjson.NotifyNewTransactionsCmd:
...
type rawResponse struct {
	...
	Error  *hdfjson.RPCError `json:"error"`
}
...
// returning an unmarshaled hdfjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]hdfjson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := hdfjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := hdfjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	// to a hdfd backend if it does exist.
	info, err := c.GetInfo()

	switch err := err.(type) {
	// Parse the hdfd version and cache it.
	case nil:
		log.Debugf("Detected hdfd version: %v", info.Version)
		version := Hdfd
	...
	case *hdfjson.RPCError:
		if err.Code != hdfjson.ErrRPCMethodNotFound.Code {
			return 0, fmt.Errorf("unable to detect hdfd version: "+
				"%v", err)
		}

	default:
		return 0, fmt.Errorf("unable to detect hdfd version: %v", err)
	}
