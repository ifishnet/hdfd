...
func (c *Client) GenerateAsync(numBlocks uint32) FutureGenerateResult {
	cmd := hdfjson.NewGenerateCmd(numBlocks)
...
func (c *Client) GenerateToAddressAsync(numBlocks int64, address hdfutil.Address, maxTries *int64) FutureGenerateToAddressResult {
	cmd := hdfjson.NewGenerateToAddressCmd(numBlocks, address.EncodeAddress(), maxTries)
	return c.sendCmd(cmd)
}

// GenerateToAddress generates numBlocks blocks to the given address and returns their hashes.
func (c *Client) GenerateToAddress(numBlocks int64, address hdfutil.Address, maxTries *int64) ([]*chainhash.Hash, error) {
...
func (c *Client) SetGenerateAsync(enable bool, numCPUs int) FutureSetGenerateResult {
	cmd := hdfjson.NewSetGenerateCmd(enable, &numCPUs)
	return c.sendCmd(cmd)
}
...
func (c *Client) GetHashesPerSecAsync() FutureGetHashesPerSecResult {
	cmd := hdfjson.NewGetHashesPerSecCmd()
	return c.sendCmd(cmd)
}
...
// information.
func (r FutureGetMiningInfoResult) Receive() (*hdfjson.GetMiningInfoResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var infoResult hdfjson.GetMiningInfoResult
...
func (c *Client) GetMiningInfoAsync() FutureGetMiningInfoResult {
	cmd := hdfjson.NewGetMiningInfoCmd()
	return c.sendCmd(cmd)
}

// GetMiningInfo returns mining information.
func (c *Client) GetMiningInfo() (*hdfjson.GetMiningInfoResult, error) {
...
func (c *Client) GetNetworkHashPSAsync() FutureGetNetworkHashPS {
	cmd := hdfjson.NewGetNetworkHashPSCmd(nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) GetNetworkHashPS2Async(blocks int) FutureGetNetworkHashPS {
	cmd := hdfjson.NewGetNetworkHashPSCmd(&blocks, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) GetNetworkHashPS3Async(blocks, height int) FutureGetNetworkHashPS {
	cmd := hdfjson.NewGetNetworkHashPSCmd(&blocks, &height)
	return c.sendCmd(cmd)
}
...
// data to work on.
func (r FutureGetWork) Receive() (*hdfjson.GetWorkResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var result hdfjson.GetWorkResult
	err = json.Unmarshal(res, &result)
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetWork() (*hdfjson.GetWorkResult, error) {
...
func (c *Client) GetWorkSubmitAsync(data string) FutureGetWorkSubmit {
	cmd := hdfjson.NewGetWorkCmd(&data)
	return c.sendCmd(cmd)
}
...
func (c *Client) SubmitBlockAsync(block *hdfutil.Block, options *hdfjson.SubmitBlockOptions) FutureSubmitBlockResult {
	blockHex := ""
	if block != nil {
	...
	cmd := hdfjson.NewSubmitBlockCmd(blockHex, options)
	return c.sendCmd(cmd)
}

// SubmitBlock attempts to submit a new block into the bitcoin network.
func (c *Client) SubmitBlock(block *hdfutil.Block, options *hdfjson.SubmitBlockOptions) error {
