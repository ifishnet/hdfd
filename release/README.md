# `hdfd`'s Reproducible Build System

This package contains the build script that the `hdfd` project uses in order to
...
and `cmd/hdfctl/version.go`.
For example (taken from [f3ec130](https://github.com/ifishnet/hdfd/commit/f3ec13030e4e828869954472cbc51ac36bee5c1d)):
```diff
diff --git a/cmd/hdfctl/version.go b/cmd/hdfctl/version.go
index 2195175c71..f65cacef7e 100644
--- a/cmd/hdfctl/version.go
+++ b/cmd/hdfctl/version.go
...
1. `git clone https://github.com/ifishnet/hdfd.git`
2. `cd hdfd`
3. `./release/release.sh <TAG> # <TAG> is the name of the next release/tag`

This will then create a directory of the form `hdfd-<TAG>` containing archives
...
Now that the directory `hdfd-<TAG>` is created, the manifest file needs to be
...
which roughly follow the format of [previous release notes](https://github.com/ifishnet/hdfd/releases/tag/v0.20.1-beta).
...
When following the GitHub documentation, include every file in the `hdfd-<TAG>`
directory.
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes. 
6. Obtain a copy of `hdfd`'s source code with `git clone
   https://github.com/ifishnet/hdfd` and checkout the source code of the
...
8. Extract the archive found in the `hdfd-<TAG>` directory created by the
   release script and recompute the `SHA256` hash of the release binaries (hdfd
   and hdfctl) with `shasum -a 256 <filename>`. These should match __exactly__
