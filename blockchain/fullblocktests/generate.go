...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: hdfutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := hdfcec.PrivKeyFromBytes(hdfcec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := hdfutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*hdfutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, hdfutil.NewTx(tx))
	}
...
func additionalCoinbase(amount hdfutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee hdfutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee hdfutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee hdfutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.HdfEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.HdfEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
