...
// oldHdfdHomeDir returns the OS specific home directory hdfd used prior to
// version 0.3.3.  This has since been replaced with hdfutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldHdfdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "hdfd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".hdfd")
	}
...
// location prior to hdfd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to hdfd
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldHdfdHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "hdfd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "hdfd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "hdfd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to hdfd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		hdfdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old hdfd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			hdfdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to hdfd as new versions require it.
