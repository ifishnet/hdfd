...
const (
	...
	MinHighPriority = hdfutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr hdfutil.Address) (*hdfutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *hdfutil.Tx, height int32) error {
...
func logSkippedDeps(tx *hdfutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress hdfutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*hdfutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := hdfutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := hdfutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A hdfutil.Block should use saved in the state to avoid
