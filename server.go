...
import (
	...
	"github.com/ifishnet/hdfd/addrmgr"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a hdfutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := hdfutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a hdfutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := hdfutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > hdfutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, hdfutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *hdfutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			hdfdLookup, func(addrs []*wire.NetAddress) {
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *hdfutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
