...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if hdfjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *hdfjson.RPCError {
				return hdfjson.NewRPCError(hdfjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := hdfjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := hdfjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := hdfjson.Error{ErrorCode: hdfjson.ErrInvalidType}
	_, err = hdfjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(hdfjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = hdfjson.MarshalResponse(1, make(chan int), nil)
