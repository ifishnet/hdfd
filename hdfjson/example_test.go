...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to hdfjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := hdfjson.NewGetBlockCmd(blockHash, hdfjson.Int(0))
	...
	id := 1
	marshalledBytes, err := hdfjson.MarshalCmd(id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*hdfjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := hdfjson.MarshalResponse(1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
