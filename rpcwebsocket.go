...
import (
	...
	"github.com/ifishnet/hdfd/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *hdfutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *hdfutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *hdfutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a hdfutil.Address) {
	switch a := a.(type) {
	case *hdfutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *hdfutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *hdfutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := hdfutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a hdfutil.Address) bool {
	switch a := a.(type) {
	case *hdfutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a hdfutil.Address) {
	switch a := a.(type) {
	case *hdfutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected hdfutil.Block
type notificationBlockDisconnected hdfutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *hdfutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := hdfjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := hdfjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *hdfutil.Block) {
	...
	ntfn := hdfjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := hdfjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := hdfjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *hdfutil.Tx) {
	...
	ntfn := hdfjson.NewTxAcceptedNtfn(txHashStr, hdfutil.Amount(amount).ToHDF())
	marshalledJSON, err := hdfjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *hdfjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = hdfjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = hdfjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*hdfutil.Tx)
	for _, op := range ops {
...
// blockDetails creates a BlockDetails struct to include in hdfws notifications
// from a block and a transaction's block index.
func blockDetails(block *hdfutil.Block, txIndex int) *hdfjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &hdfjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *hdfutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := hdfjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return hdfjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *hdfutil.Tx, block *hdfutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := hdfjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *hdfutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := hdfjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := hdfjson.MarshalCmd(nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *hdfutil.Tx, block *hdfutil.Block) {
...
out:
	for {
		...
		if err != nil {
			...
			jsonErr := &hdfjson.RPCError{
				Code:    hdfjson.ErrRPCParse.Code,
		...
		//
		// Hdfd does not respond to any request without and "id" or "id":null,
		...
		switch authCmd, ok := cmd.cmd.(*hdfjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &hdfjson.RPCError{
					Code:    hdfjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.HelpCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &hdfjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.NotifySpentCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.NotifyReceivedCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.StopNotifySpentCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := hdfutil.DecodeAddress(addr, params)
		if err != nil {
			return &hdfjson.RPCError{
				Code: hdfjson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []hdfjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = hdfjson.RPCError{
	Code:    hdfjson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *hdfutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := hdfjson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *hdfutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hdfjson.RescanBlocksCmd)
	if !ok {
		return nil, hdfjson.ErrRPCInternal
	}
	...
	discoveredData := make([]hdfjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &hdfjson.RPCError{
				Code:    hdfjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, hdfjson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *hdfutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *hdfjson.RescanCmd, lookups *rescanKeys, minBlock,
	maxBlock int32, chain *blockchain.BlockChain) (
	*hdfutil.Block, *chainhash.Hash, error) {
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &hdfjson.RPCError{
				Code:    hdfjson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := hdfjson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := hdfjson.MarshalCmd(nil, n)
			if err != nil {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	// Notify websocket client of the finished rescan.  Due to how hdfd
