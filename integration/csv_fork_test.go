...
import (
	...
	"github.com/ifishnet/hdfd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt hdfutil.Amount) (*hdfcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := hdfutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	hdfdCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, hdfdCfg)
	if err != nil {
	...
	const outputValue = hdfutil.SatoshiPerBitcoin
	...
	txns := []*hdfutil.Tx{hdfutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis hdfutil.Amount, timeLock int32,
	...
	p2shAddr, err := hdfutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = hdfutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*hdfutil.Tx{hdfutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*hdfutil.Tx{hdfutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
