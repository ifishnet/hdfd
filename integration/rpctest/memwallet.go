...
import (
	...
	"github.com/ifishnet/hdfd/rpcclient"
...
type memWallet struct {
	coinbaseKey  *hdfcec.PrivateKey
	coinbaseAddr hdfutil.Address
	...
	addrs map[uint32]hdfutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]hdfutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*hdfutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          hdfutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (hdfutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []hdfutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt hdfutil.Amount,
	feeRate hdfutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := hdfutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate hdfutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate hdfutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt hdfutil.Amount
	for _, output := range outputs {
		outputAmt += hdfutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() hdfutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance hdfutil.Amount
...
func keyToAddr(key *hdfcec.PrivateKey, net *chaincfg.Params) (hdfutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := hdfutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
