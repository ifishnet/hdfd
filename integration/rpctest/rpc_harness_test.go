...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt hdfutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(hdfutil.Amount(5 * hdfutil.SatoshiPerBitcoin))
	...
	txid = genSpend(hdfutil.Amount(500 * hdfutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(hdfutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*hdfutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, hdfutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := hdfutil.Amount(250 * hdfutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = hdfutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := hdfutil.Amount(50 * hdfutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := hdfutil.Amount(numMatureOutputs * 50 * hdfutil.SatoshiPerBitcoin)
