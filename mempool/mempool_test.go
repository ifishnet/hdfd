// Copyright (c) 2016 The ifishnet developers
...
func (s *fakeChain) FetchUtxoView(tx *hdfutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *hdfutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *hdfutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   hdfutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
type poolHarness struct {
	...
	signKey     *hdfcec.PrivateKey
	payAddr     hdfutil.Address
	payScript   []byte
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*hdfutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee hdfutil.Amount,
	signalsReplacement bool) (*hdfutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput hdfutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*hdfutil.Tx, error) {
	txChain := make([]*hdfutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, hdfutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := hdfcec.PrivKeyFromBytes(hdfcec.S256(), keyBytes)

	// Generate associated pay-to-script-hash address and resulting payment
	// script.
	pubKeyBytes := signPub.SerializeCompressed()
	payPubKeyAddr, err := hdfutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *hdfutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee hdfutil.Amount,
	signalsReplacement, confirmed bool) *hdfutil.Tx {
...
func testPoolMembership(tc *testContext, tx *hdfutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   hdfutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *hdfutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*hdfutil.Tx, []*hdfutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*hdfutil.Tx, []*hdfutil.Tx) {
				...
				return tx, []*hdfutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*hdfutil.Tx, []*hdfutil.Tx) {
				...
				return tx, []*hdfutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = hdfutil.SatoshiPerBitcoin
