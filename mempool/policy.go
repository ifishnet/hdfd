...
const (
	...
	DefaultMinRelayTxFee = hdfutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee hdfutil.Amount) int64 {
	...
	if minFee < 0 || minFee > hdfutil.MaxSatoshi {
		minFee = hdfutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *hdfutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee hdfutil.Amount) bool {
...
func checkTransactionStandard(tx *hdfutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee hdfutil.Amount,
...
func GetTxVirtualSize(tx *hdfutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
