...
// newAddressPubKey returns a new hdfutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) hdfutil.Address {
	addr, err := hdfutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new hdfutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) hdfutil.Address {
	addr, err := hdfutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new hdfutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) hdfutil.Address {
	addr, err := hdfutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the hdfutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the hdfutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := hdfutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := hdfutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := hdfutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := hdfutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := hdfutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*hdfutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*hdfutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*hdfutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*hdfutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*hdfutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
