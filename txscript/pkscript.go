...
const (
	...
	//   Public key (33 byte)
	minPubKeyHashSigScriptLen = 1 + hdfcec.MinSigLen + 1 + 1 + 33
...
func (s PkScript) Address(chainParams *chaincfg.Params) (hdfutil.Address, error) {
...
func computeNonWitnessPkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) >= minPubKeyHashSigScriptLen &&
		...
		if hdfcec.IsCompressedPubKey(pubKey) {
