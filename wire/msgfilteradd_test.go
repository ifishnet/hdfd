...
func TestFilterAddLatest(t *testing.T) {
	...
	err := msg.HdfEncode(&buf, pver, enc)
	if err != nil {
	...
	err = readmsg.HdfDecode(&buf, pver, enc)
	if err != nil {
...
func TestFilterAddCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.HdfEncode(&buf, ProtocolVersion, LatestEncoding)
	if err != nil {
		t.Errorf("encode of MsgFilterAdd failed %v err <%v>", msg, err)
	}
	...
	var readmsg MsgFilterAdd
	err = readmsg.HdfDecode(&buf, BIP0031Version, LatestEncoding)
	if err == nil {
...
func TestFilterAddMaxDataSize(t *testing.T) {
	...
	err = msg.HdfDecode(readbuf, ProtocolVersion, LatestEncoding)
	if err == nil {
		t.Errorf("decode of MsgFilterAdd succeeded when it shouldn't "+
			"have %v", msg)
	}
