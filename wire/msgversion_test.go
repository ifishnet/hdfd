...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.HdfDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.HdfDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.HdfDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.HdfDecode(buf, test.pver, test.enc)
