...
type Message interface {
	HdfDecode(io.Reader, uint32, MessageEncoding) error
	HdfEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func WriteMessageN(w io.Writer, msg Message, pver uint32, hdfnet BitcoinNet) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, hdfnet, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32, hdfnet BitcoinNet) error {
	_, err := WriteMessageN(w, msg, pver, hdfnet)
	return err
}
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	hdfnet BitcoinNet, encoding MessageEncoding) (int, error) {
	...
	var bw bytes.Buffer
	err := msg.HdfEncode(&bw, pver, encoding)
	if err != nil {
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, hdfnet BitcoinNet,
	...
	// MsgVersion HdfDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.HdfDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32, hdfnet BitcoinNet) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, hdfnet, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32, hdfnet BitcoinNet) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver, hdfnet)
	return msg, buf, err
}
