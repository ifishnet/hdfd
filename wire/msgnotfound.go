...
func (msg *MsgNotFound) HdfDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgNotFound.HdfDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgNotFound) HdfEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgNotFound.HdfEncode", str)
	}
