...
func (msg *MsgCFHeaders) HdfDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.HdfDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) HdfEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.HdfEncode", str)
	}
...
// differs from HdfDecode in that HdfDecode decodes from the bitcoin wire
