// Copyright (c) 2014-2015 The ifishnet developers
...
func (msg *MsgFilterClear) HdfDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterclear message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterClear.HdfDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgFilterClear) HdfEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterclear message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterClear.HdfEncode", str)
	}
