...
func TestMsgAlert(t *testing.T) {
	...
	// Test HdfEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.HdfEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("HdfEncode got: %s want: %s",
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.HdfEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("HdfEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("HdfEncode #%d wrong error got: %v, "+
		...
		err = msg.HdfDecode(r, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("HdfDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("HdfDecode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.HdfEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.HdfEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.HdfEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.HdfEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
