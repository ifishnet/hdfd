...
const (
	...
	unusableFlags = hdfjson.UFWebsocketOnly | hdfjson.UFNotification
)

var (
	hdfdHomeDir           = hdfutil.AppDataDir("hdfd", false)
	hdfctlHomeDir         = hdfutil.AppDataDir("hdfctl", false)
	hdfwalletHomeDir      = hdfutil.AppDataDir("hdfwallet", false)
...
func listCommands() {
	...
	cmdMethods := hdfjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := hdfjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := hdfjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&hdfjson.UFWalletOnly != 0 {
...
func normalizeAddress(addr string, chain *chaincfg.Params, useWallet bool) (string, error) {
	...
	if err != nil {
		...
		case &chaincfg.RegressionNetParams:
			if useWallet {
				// TODO: add port once regtest is supported in hdfwallet
				paramErr := fmt.Errorf("cannot use -wallet with -regtest, hdfwallet not yet compatible with regtest")
				return "", paramErr
			} else {
				defaultPort = "18334"
			}
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(hdfctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default hdfctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(hdfwalletHomeDir, "hdfwallet.conf")
		} else {
			serverConfigPath = filepath.Join(hdfdHomeDir, "hdfd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either hdfd or
// hdfwallet), and extract the RPC user and password from it.
