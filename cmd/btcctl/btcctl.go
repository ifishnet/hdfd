...
func main() {
	...
	method := args[0]
	usageFlags, err := hdfjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := hdfjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// hdfjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(hdfjson.Error); ok {
		...
		// The error is not a hdfjson.Error and this really should not
	...
	marshalledJSON, err := hdfjson.MarshalCmd(1, cmd)
	if err != nil {
